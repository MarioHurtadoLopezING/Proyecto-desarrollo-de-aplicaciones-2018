...
public class ActividadFacadeREST extends AbstractFacade<Actividad> {
    ...
    @GET
    @Path("/actividadesCurso/{idCurso}")//
    @Produces({MediaType.APPLICATION_JSON})
    public List<Actividad> buscarActividadesCurso(@PathParam("idCurso") int  idCurso){
        Query query = getEntityManager().createNamedQuery("Actividad.findbyIdCurso");
        query.setParameter("idCurso", idCurso);
        List<Actividad> user = query.getResultList();
        return user;
    }
    @GET
    @Path("{id}")
